...
async function fetchOfficialHolidays(year) {
  try {
    const r = await fetch(`/api/stocks?type=holidays&year=${year}`, { signal: AbortSignal.timeout(8000) });
...
// ----------------------------------------------------------------
//  API fetch — 呼叫 Vercel serverless function (/api/stocks)
// ----------------------------------------------------------------
async function apiFetch(type) {
  try {
    const r = await fetch(`/api/stocks?type=${type}`, {
      signal: AbortSignal.timeout(12000),
    });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    return await r.json();
  } catch (e) {
    console.warn('apiFetch failed:', type, e.message);
    return null;
  }
}
...
//  TWSE API
// ----------------------------------------------------------------
async function fetchTWSEDisposal()  { return apiFetch('twse-disposal');  }
async function fetchTWSEAttention() { return apiFetch('twse-attention'); }

// ----------------------------------------------------------------
//  TPEx API
// ----------------------------------------------------------------
async function fetchTPExDisposal()  { return apiFetch('tpex-disposal');  }
...
async function fetchTPExAttention() { return apiFetch('tpex-attention'); }
