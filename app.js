...
async function fetchOfficialHolidays(year) {
  try {
    ...
    if (!data || String(data.stat || '').toUpperCase() !== 'OK' || !Array.isArray(data.data)) return null;
...
//  API fetch helpers
// ----------------------------------------------------------------
// ✅ 你自己的 Cloudflare Worker（備用，openapi 失敗才用）
const CF_WORKER_URL = 'https://shy-tooth-36f4.linj50647.workers.dev';

const PROXIES = [
  // 🥇 自己的 Cloudflare Worker
  url => ({ url: `${CF_WORKER_URL}?url=${encodeURIComponent(url)}`, type: 'direct' }),
  // corsproxy.io
  url => ({ url: `https://corsproxy.io/?url=${encodeURIComponent(url)}`, type: 'direct' }),
  // allorigins — 回應包在 .contents
  url => ({ url: `https://api.allorigins.win/get?url=${encodeURIComponent(url)}`, type: 'allorigins' }),
  // cors.sh
  url => ({ url: `https://proxy.cors.sh/${url}`, type: 'direct', headers: { 'x-cors-api-key': 'temp_' + Date.now() } }),
  // codetabs
  url => ({ url: `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(url)}`, type: 'direct' }),
  // cors-anywhere
  url => ({ url: `https://cors-anywhere.herokuapp.com/${url}`, type: 'direct', headers: { 'Origin': 'https://disposition-radar.app' } }),
];

async function apiFetch(url, timeout = 9000) {
  // 1. 直接請求（有些 API 支援 CORS）
  try {
  ...
  // 2. 依序嘗試各 CORS Proxy（使用 Promise.any 加速）
  // 先並發試前2個最穩定的
  const fastProxies = PROXIES.slice(0, 2);
  try {
    const result = await Promise.any(
      fastProxies.map(async makeProxy => {
        const { url: proxyUrl, type, headers: extraHeaders } = makeProxy(url);
        ...
        if (type === 'allorigins' && typeof w.contents === 'string') {
          return JSON.parse(w.contents);
        }
        // 驗證是真實資料
        if (w && (w.stat !== undefined || w.data !== undefined || w.aaData !== undefined || Array.isArray(w))) return w;
  ...
  // 3. 逐一嘗試剩餘 proxy
  for (const makeProxy of PROXIES.slice(2)) {
    try {
      const { url: proxyUrl, type, headers: extraHeaders } = makeProxy(url);
      const r = await fetch(proxyUrl, {
      ...
      if (r.ok) {
        const text = await r.text();
        let w;
        try { w = JSON.parse(text); } catch(_) { continue; }
        if (type === 'allorigins' && typeof w.contents === 'string') {
          try { return JSON.parse(w.contents); } catch(_) {}
        }
        if (w && (w.stat !== undefined || w.data !== undefined || w.aaData !== undefined || Array.isArray(w))) return w;
        // 寬鬆條件：只要是物件就接受
...
// ----------------------------------------------------------------
//  TWSE API
//  ✅ 優先使用 openapi.twse.com.tw（原生支援 CORS，不需 proxy）
//     備用才走 www.twse.com.tw（需 proxy）
// ----------------------------------------------------------------
async function fetchTWSEDisposal() {
  // 先試官方 OpenAPI（CORS-friendly）
  const openApiUrl = 'https://openapi.twse.com.tw/v1/surveillance/disposalStockList';
  try {
    const r = await fetch(openApiUrl, { signal: AbortSignal.timeout(8000), headers: { 'Accept': 'application/json' } });
    if (r.ok) { const d = await r.json(); if (d && (Array.isArray(d) || d.data || d.stat)) return d; }
  } catch(_) {}
  // 備用：走 proxy
...
async function fetchTWSEAttention() {
  // 先試官方 OpenAPI（CORS-friendly）
  const openApiUrl = 'https://openapi.twse.com.tw/v1/surveillance/attentionStockList';
  try {
    const r = await fetch(openApiUrl, { signal: AbortSignal.timeout(8000), headers: { 'Accept': 'application/json' } });
    if (r.ok) { const d = await r.json(); if (d && (Array.isArray(d) || d.data || d.stat)) return d; }
  } catch(_) {}
...
async function fetchTPExDisposal() {
  const t = Date.now();
  const urls = [
    `https://www.tpex.org.tw/web/stock/supervision/disposal/disposal_stock_result.php?l=zh-tw&o=json&_=${t}`,
    `https://www.tpex.org.tw/www/zh-tw/supervision/disposal?response=json&_=${t}`,
    `https://www.tpex.org.tw/web/stock/supervision/disposal/disposal_stock_result.php?l=zh-tw&o=json`,
    `https://www.tpex.org.tw/web/stock/supervision/disposal/disposal_stock_result.php?o=json&l=zh-tw`,
  ];
...
function parseTWSEDisposal(data) {
  if (!data || String(data.stat || '').toUpperCase() !== 'OK' || !Array.isArray(data.data)) return [];
...
async function fetchTPExAttention() {
  const t = Date.now();
  const urls = [
    `https://www.tpex.org.tw/web/stock/supervision/attention/attention_stock_result.php?l=zh-tw&o=json&_=${t}`,
    `https://www.tpex.org.tw/www/zh-tw/supervision/attention?response=json&_=${t}`,
    `https://www.tpex.org.tw/web/stock/supervision/attention/attention_stock_result.php?l=zh-tw&o=json`,
    `https://www.tpex.org.tw/web/stock/supervision/attention/attention_stock_result.php?o=json&l=zh-tw`,
  ];
  for (const url of urls) {
    const data = await apiFetch(url);
    if (data) return data;
  ...
  if (!stocks.length) {
    container.innerHTML = `<div class="empty-state">
        ...
        <div style="font-size:12px;color:var(--muted);margin-top:8px;line-height:1.6">
          若長時間無資料，可能是 CORS 代理暫時失效。<br>
          請點右上角 <strong>↻</strong> 重試，或稍後再回來。
        </div>
      </div>`;
    updateSummary();
    return;
...
async function loadData() {
  ...
  if (cached && ageMin < 20) {
    try {
      ...
      // 還原快取時重新計算 remaining（快取存的是昨天的舊數字，今天必須重算）
      const recalcFromCache = arr => (arr || []).map(s => ({
        ...s,
        startDate: s.startDate ? new Date(s.startDate) : null,
        endDate:   s.endDate   ? new Date(s.endDate)   : null,
        remaining: remainingTradingDays(s.endDate ? new Date(s.endDate) : null),
      })).filter(s => s.remaining >= 0);
      allStocks.twse      = recalcFromCache(parsed.twse);
      allStocks.tpex      = recalcFromCache(parsed.tpex);
...
async function freshFetch() {
  ...
  try {
    ...
    if (twseRes.status === 'fulfilled' && twseRes.value) {
      ...
      try { const c = safeGet('stockCache'); allStocks.twse = c ? (JSON.parse(c).twse || []) : []; } catch(_){ allStocks.twse=[]; }
      showToast('⚠️ 上市(TWSE)連線失敗，已嘗試多條備用線路');
    }
    ...
    if (tpexRes.status === 'fulfilled' && tpexRes.value) {
      ...
      try { const c = safeGet('stockCache'); allStocks.tpex = c ? (JSON.parse(c).tpex || []) : []; } catch(_){ allStocks.tpex=[]; }
      if (allStocks.tpex.length === 0) showToast('⚠️ 上櫃(TPEx)連線失敗');
    }
    ...
    if (container) {
      container.innerHTML = `<div class="empty-state">
        ...
        <div style="font-size:12px;color:var(--muted);margin-top:8px;line-height:1.8">
          可能原因：網路不穩定 / CORS 代理服務暫時中斷<br>
          <strong>建議：</strong>稍等 1–2 分鐘後點 ↻ 重試<br>
          錯誤訊息：${e.message || '網路錯誤'}
        </div>
      </div>`;
    }
    updateTabBadges();
    showToast('❌ 所有連線管道失敗，請稍後重試', 5000);
  }
}
...
function showLoading() {
  document.getElementById('stock-container').innerHTML = `
    <div class="loading-card">
      ...
      <div class="loading-sub">自動嘗試多條備用線路，請稍候（約 10–20 秒）</div>
    </div>`;
}
