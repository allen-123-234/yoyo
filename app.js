...
async function apiFetch(url, timeout = 9000) {
  ...
  for (const makeProxy of PROXIES) {
    try {
      ...
      const r = await fetch(proxyUrl, { signal: AbortSignal.timeout(timeout + 4000) });
      if (r.ok) {
        ...
        // codetabs 直接回傳 JSON
        if (w && (w.data !== undefined || w.stat !== undefined || Array.isArray(w))) return w;
        // 其餘 proxy 直接回傳
        return w;
      }
    } catch (_) {}
