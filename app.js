...
async function fetchOfficialHolidays(year) {
  try {
    const target = `https://www.twse.com.tw/rwd/zh/holidaySchedule/holidaySchedule?response=json&queryYear=${year}&_=${Date.now()}`;
...
async function workerFetch(targetUrl) {
  try {
    ...
    console.warn('workerFetch failed:', targetUrl, e.message);
    return null;
  }
}
...
async function fetchTWSEDisposal() {
  return workerFetch('https://www.twse.com.tw/rwd/zh/surveillance/disposalStockList?response=json&_=' + Date.now());
}

async function fetchTWSEAttention() {
  return workerFetch('https://www.twse.com.tw/rwd/zh/surveillance/attentionStockList?response=json&_=' + Date.now());
}
...
async function fetchTPExDisposal() {
  return workerFetch('https://www.tpex.org.tw/web/stock/supervision/disposal/disposal_stock_result.php?l=zh-tw&o=json&_=' + Date.now());
}
...
async function fetchTPExAttention() {
  return workerFetch('https://www.tpex.org.tw/web/stock/supervision/attention/attention_stock_result.php?l=zh-tw&o=json&_=' + Date.now());
}
