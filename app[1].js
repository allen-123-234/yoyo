...
async function ensureHolidaysCoverYear(targetYear) {
  const storedMax = parseInt(localStorage.getItem('holidaysMaxYear') || HOLIDAYS_CONFIRMED_THRU);
  const storedExtra = JSON.parse(localStorage.getItem('holidaysExtra') || '[]');
  ...
  localStorage.setItem('holidaysExtra', JSON.stringify(allExtra));
  localStorage.setItem('holidaysMaxYear', targetYear.toString());
}
...
//  State
// ----------------------------------------------------------------
let extraClosures   = new Set(JSON.parse(localStorage.getItem('extraClosures') || '[]'));
let typhoonMode     = localStorage.getItem('typhoonMode') === 'true';
let notifEnabled    = localStorage.getItem('notifEnabled') === 'true';
let autoUpdate      = localStorage.getItem('autoUpdate') === 'true';
let currentTab      = 'twse';
let manualStocks    = (JSON.parse(localStorage.getItem('manualStocks') || '[]')).map(s => ({
  ...s,
...
async function loadData() {
  ...
  const cacheTime = parseInt(localStorage.getItem('stockCacheTime') || '0');
...
function saveCache() {
  localStorage.setItem('stockCache', JSON.stringify(allStocks));
  localStorage.setItem('stockCacheTime', Date.now().toString());
}
...
window.toggleTyphoon = function() {
  ...
  localStorage.setItem('typhoonMode', typhoonMode);
...
window.clearCache = function() {
  localStorage.removeItem('stockCache');
  localStorage.removeItem('stockCacheTime');
  freshFetch();
